...
const RequestTypeSection = ({ pageTitle = "" }) => {
  ...
  // Check if this is an approval page (routes /3, /4, /5)
  const isApprovalPage = isRMPMApprovalPage || isCabCoordinatorPage || isTransportReportPage;
  ...
  return (
    <div className="request-type-container">
      <div className="request-type-header">
        <label className="request-type-label">{isApprovalPage ? 'Cab Type' : 'Request Type'}</label>
      ...
      {showNoteModal && (
        <>
          ...
          <div className={`request-note-tooltip ${isTransportReportPage ? 'transport-report-note' : ''}`}>
